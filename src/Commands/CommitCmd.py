...
class CommitCmd(NeedsData):
    ...
    def run(self) -> None:
        ...
        :return: None
        """
        storage_file = open(f"data\\Data\\{self.data.name}.json", 'w')
        weights_file = open(f"data\\Weights\\{self.data.name}_weights.json", 'w')

        json.dump(self.data.get_data_dict(), storage_file)
        json.dump(self.data.get_weights_dict(), weights_file)
    ...
    def set_overall(self):
        ...
        output = open("data\\Overall Averages.txt", 'w')
        output.write(data)
        output.flush()
        output.close()
    ...
    def get_analysis(self) -> str:
        ...
        """
        input_file = open("data\\Overall Averages.txt", 'r')
